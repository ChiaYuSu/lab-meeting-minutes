...
與 OpenFlow 一樣是南向接口 (SouthBound Interface)，但交換機在處理封包的能力更加強大。而 P4 在設計時基於以下幾點：**(1) 可重構性(Reconfigurability)**：開發人員可以隨時改變封包的處理並且在編譯後配置到交換機、**(2) 協議無關性(Protocol Independent)**：網路設備無須與其他協議綁定，使用人員可以使用 P4 語言描述任何網路數據平面協議及封包的處理。至於 P4 Language 怎麼寫還有規則該如何寫，因為時間有限，大家可以去官網查一下 (p4.org)
