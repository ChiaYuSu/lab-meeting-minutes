...
在當今社會，考慮數據的安全性和保護數據的有效性是非常重要的。關於安全數據領域，已經進行了多項研究。然而，由於安全需求的增加和對訊息交換的影響的增加，提出了一種新的基於二維碼 (QR code) 的 RSA 安全加密方法。本文介紹了一種基於 QR code 的 RSA 演算法，該算法可用於銀行客戶的數據安全共享。用戶需要掃描 QR code，獲取存儲在 QR code 中的個人訊息。本文闡述了系統的實現細節。此外，它還考慮了系統如何驗證每個用戶的身份，以消除虛假注冊。

## 介紹
<img src="1090730\1090730.007.jpeg" width="550px" />

在當今，保護和隱藏個人機密訊息已成為一項挑戰。由於缺乏安全性和機密性，機密訊息偽造會導致人員損失巨大利潤。如今，在一個人與另一個人之間共享個人訊息變得很重要。在此問題上，最流行的示例之一是在某些國家銀行訊息的交換，例如，通常在機構和客戶之一之間交換一些銀行訊息，以解決促進銀行業務的問題。由於某些銀行或其他機構允許此過程使所有用戶看到某些銀行業務，例如將資金轉入其他帳戶並知道餘額。銀行還允許您通過發送給用戶的簡訊 (SMS) 向他們提供最新的銀行業務擴展以及每家銀行提供的類似優惠。近年來，通過簡訊入侵竊取銀行訊息的現象越來越多並迅速蔓延。所以在本文中，提出了一種將一個人與另一個人之間的訊息加密的系統，目的在通過將 QR code 和 RSA 演算法結合在一起來確保 QR code 中更高的安全性。
...
QR code 最近已經廣泛傳播。由於任何手機都可以掃 QR code，因此訪問 QR 碼變得非常容易。因此，本文介紹了一種基於圖像加密 (QR Code) 的加密和解密方法。

## RSA 加密演算法
<img src="1090730\1090730.011.jpeg" width="550px" />

在基礎密碼學中，有兩種加解密方式，分別式對稱式加密 (Symmetric Encryption) 與非對稱式加密 (Asymmetric Encryption)。對稱式加密就是 sender 與 receiver 的加解密皆使用同一把金鑰，所以只要雙方都擁有這把鑰匙，當 sender 傳資料時，使用這把鑰匙加密，receiver 收到訊息後，再用同一把鑰匙解密，就能解開訊息了，運作原理如上圖所示。

但對稱式加密最大的弱點是，要是這把鑰匙在第一次傳送時，被中間人攔截且複製，再原封不動傳給接收方，那之後所有攔截到的加密訊息，都能被輕易破解，因此就需要更進階的非對稱式加密技術去解決這個 bug。

---
<img src="1090730\1090730.012.jpeg" width="550px" />

非對稱式加密就是每個使用者都擁有一對金鑰：公開金鑰 (Public key) 及私密金鑰 (Private key)，公開金鑰能被廣泛的發佈與流傳，而私密金鑰則必須被妥善的保存；當訊息由其中一把金鑰加密後，就必須用另一把金鑰解密，加解密的鑰匙要是完整一對 (pair) 的。運作原理是 sender 與 receiver 在傳送之前，先把彼此的公鑰傳給對方，當傳送方要傳送時，就用 receiver的公鑰將訊息加密，receiver 收到加密訊息後，再用自己的私鑰解開，這樣即使有心人拿到公鑰，只要沒拿到接收方的私鑰，也還是無法解密訊息。
...
這時就需要數位簽章 (Digital Signature) 的 Double Confirm 了，也就是 sender 除了使用 receiver 的公鑰加密外，也使用自己的私鑰對該封加密訊息的 Hash 簽名，這樣當 receiver 收到時，除了要使用 receiver 本身的私鑰解密，也需要用 sender 的公鑰來對簽章作二次驗證，確認真的是由正確的 sender 傳來的。

簡單總結複習一下概念：<br>
Q：我要傳一個訊息給小明，要怎麼只讓小明看到，且讓他確認該封訊息就是我傳給他的呢？<br>
Ans：一開始我跟小明先交換公開金鑰 (這樣我們都有對方的金鑰)，然後我用小明的公鑰把訊息加密，再用我自己的私鑰對該封訊息簽名 (數位簽章)，小明收到後，先用我的公鑰確認這個簽名是我的沒錯，再用他的私鑰解開內容，便完成了訊息只有小明能看到，且確認是我本人寄給他的雙重保障。
...
本論文提出的方案是 QR code 與 RSA 演算法的組合。

該系統致力於通過將 QR code 與 RSA 演算法結合使用來增強安全性要求，提出了兩層體系結構。
...
當 receiver 收到 QR 碼時，QR 掃描器將處理 QR 碼以獲取數據 (加密數據)，然後透過 RSA 方法解密加密數據以獲取原始數據。RSA 方法分為兩個階段 (加密和解密)。
